...
def find_img_tag(headers):
  '''
  적당한 텀을 두고 변동되는 나무위키 사이트의 class정보를 찾아내는 함수
  
  headers : dict / 유저 에이전트 정보가 담겨있는 딕셔너리
  '''
  driver = set_chrome_driver(headers)
  url = 'https://namu.wiki/w/로시츠키'
  driver.get(url=url)
  html = driver.page_source
  driver.close()
  soup = BeautifulSoup(html, 'lxml')
  tag = soup.find('img', attrs={'alt':'Tomáš Rosický'})['class'][0]
  return tag

def scrapNamuImg(key, path, headers, tag ,namuKeyword_kind='person'):
    '''
    ...
    if namuKeyword_kind == 'person':
        ...
        imglink = [il['src'] for il in soup.find_all('img', attrs={'class':tag}) if not il.find_parent('dd') and check_txt_logo(il['alt'], in_=False) ]
        ...
        # 키워드와 로고키워드가 있을 시에
        imglink = [il['src'] for il in soup.find_all('img', attrs={'class':tag}) if not il.find_parent('dd') and (key_ in  il['alt']) and check_txt_logo(il['alt']) ]
        if len(imglink) < 1:
            #로고키워드가 있을 시에
            imglink = [il['src'] for il in soup.find_all('img', attrs={'class':tag}) if not il.find_parent('dd') and check_txt_logo(il['alt'])]
        for i, link in enumerate(imglink):
            res=requests.get("https:"+link,headers=headers)
            try:
